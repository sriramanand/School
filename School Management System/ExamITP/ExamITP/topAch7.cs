...
namespace ExamITP
{
    public partial class topAch7 : MetroFramework.Forms.MetroForm
    {
        ...
        private void topAch7_Load(object sender, EventArgs e)
        {
            SqlCommand command1 = new SqlCommand();
            command1.Connection = con;
            if (metroLabel6.Text == "7")
            {
                command1.CommandText = "select AdmissionNo,fname,surname,[English],DENSE_RANK() OVER (ORDER BY English DESC) AS [Standing] from yearEnd where admissionGrade  = '7'";
            }
            else
            {
            ...
            if (metroLabel6.Text == "7")
            {
                command2.CommandText = "select AdmissionNo,fname,surname,[Mathematics],DENSE_RANK() OVER (ORDER BY Mathematics DESC) AS [Standing] from yearEnd where admissionGrade  = '7'";
            }
            else
            {
            ...
            if (metroLabel6.Text == "7")
            {
                command3.CommandText = "select AdmissionNo,fname,surname,[ICT],DENSE_RANK() OVER (ORDER BY ICT DESC) AS [Standing] from yearEnd where admissionGrade  = '7'";
            }
            else
            {
            ...
            if (metroLabel6.Text == "7")
            {
                command4.CommandText = "select AdmissionNo,fname,surname,[Science],DENSE_RANK() OVER (ORDER BY Science DESC) AS [Standing] from yearEnd where admissionGrade  = '7'";
            }
            else
            {
            ...
            if (metroLabel6.Text == "7")
            {
                command5.CommandText = "select AdmissionNo,fname,surname,[History],DENSE_RANK() OVER (ORDER BY History DESC) AS [Standing] from yearEnd where admissionGrade  = '7'";
            }
            else
            {
            ...
            if (metroLabel6.Text == "7")
            {
                command6.CommandText = "select AdmissionNo,fname,surname,[Language],DENSE_RANK() OVER (ORDER BY Language DESC) AS [Standing] from yearEnd where admissionGrade  = '7'";
            }
            else
            {
